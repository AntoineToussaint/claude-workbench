...
const TERMINAL_STRATEGIES = {
  ghostty: (bin, session, title, bgColor, fullscreen, shell) => {
    ...
    if (fullscreen) {
      lines.push(`fullscreen = true`);
      if (process.platform === "darwin") {
        lines.push(`macos-non-native-fullscreen = true`);
      }
    }
    return { bin, args: [`--config-file=${configPath}`], configFile: configPath, configContent: lines.join("\n") + "\n" };
  },

  kitty: (bin, session, title, bgColor) => ({
    bin,
    args: ["--title", title, "-o", `background=${bgColor}`, "tmux", "attach", "-t", session],
  }),

  alacritty: (bin, session, title) => ({
    bin,
    args: ["--title", title, "-e", "tmux", "attach", "-t", session],
  }),

  "gnome-terminal": (bin, session, title) => ({
    bin,
    args: [`--title=${title}`, "--", "tmux", "attach", "-t", session],
  }),

  xterm: (bin, session, title) => ({
    bin,
    args: ["-title", title, "-e", "tmux", "attach", "-t", session],
  }),

  iterm2: (_bin, session, _title) => ({
    bin: "open",
    args: ["-a", "iTerm"],
    postSpawnScript: `osascript -e 'tell application "iTerm" to tell current session of current window to write text "tmux attach -t ${session}"'`,
  }),

  "terminal.app": (_bin, session, _title) => ({
    bin: "open",
    args: ["-a", "Terminal"],
    postSpawnScript: `osascript -e 'tell application "Terminal" to do script "tmux attach -t ${session}" in front window'`,
  }),
};
 ...
 */
export function getTerminalSpawnArgs(terminalName, bin, session, title, bgColor, fullscreen, shell) {
  const strategy = TERMINAL_STRATEGIES[terminalName];
  if (!strategy) return null;
  return strategy(bin, session, title, bgColor, fullscreen, shell || process.env.SHELL || "/bin/bash");
}
