...
export class TmuxMultiplexer {
  ...
  async createSession(name, cwd, panes, config = {}) {
    ...
    if (colorDef.hex) {
      cmds.push(
        `tmux set-option -t ${name} status-style "bg=${colorDef.hex},fg=#000000"`,
        `tmux set-option -t ${name} status-left " ■ ${color.toUpperCase()} │ ${title} "`,
        `tmux set-option -t ${name} status-left-length 120`,
    ...
    cmds.push(`tmux split-window -v -b -t ${name}:.${lastPane} -c "${cwd}" -l 20% '${objectiveCmd}'`);
    paneOffset = 1;
