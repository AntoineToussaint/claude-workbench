...
export class TmuxMultiplexer {
  ...
  async createSession(name, cwd, panes, config = {}) {
    ...
    // Status bar — dark bg, color accent for name, task title on right
    const accent = colorDef.hex ?? "#ffffff";
    const safeTitle = title.replace(/"/g, '\\"');
    cmds.push(
      `tmux set-option -t ${name} status on`,
      `tmux set-option -t ${name} status-style "bg=#1a1a1a,fg=#888888"`,
      `tmux set-option -t ${name} status-left "#[fg=${accent},bold] ● ${color} #[default]"`,
      `tmux set-option -t ${name} status-left-length 30`,
      `tmux set-option -t ${name} status-right "#[fg=#aaaaaa] ${safeTitle} "`,
