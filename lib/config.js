...
export function loadConfig() {
  if (!existsSync(CONFIG_FILE)) {
    ...
    console.log(`Auto-generated workbench.config.json (detected: ${platform.terminal.name} terminal, ${platform.editor ?? "no"} editor)`);
    return config;
  }

  const config = JSON.parse(readFileSync(CONFIG_FILE, "utf-8"));

  // If launchers are missing/empty, auto-detect and merge
  if (!config.launchers || config.launchers.length === 0) {
    const platform = detectPlatform();
    const defaults = generateDefaultConfig(platform);
    config.launchers = defaults.launchers;
    writeFileSync(CONFIG_FILE, JSON.stringify(config, null, 2) + "\n");
    console.log(`Auto-detected launchers: ${config.launchers.map((l) => l.label).join(", ")}`);
  }
